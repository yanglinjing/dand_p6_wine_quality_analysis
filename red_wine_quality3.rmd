...
#### Density
```{r}
create_density(df$fixed.acidity, 'Fixed Acidity', 4, 16, 1)
```

#### Normalisation: comparing with log10, sqrt
```{r}
create_normalisation(df$fixed.acidity, 'Fixed Acidity')
```
...
```{r}
# Prepare to delete
c <- list(subset(df, !is.na(alcohol))$alcohol, 'Alcohol', 8, 15, 1)
#print(c[[2]], c[[3]])
create_density(c[[1]], c[[2]],c[[3]],c[[4]],c[[5]])
#create_density(df$alcohol, 'Alcohol', 8, 15, 1)
```
...
# Step 3: Iterate over the nested list to draw plots for each variable 
for (c in chemicals) {
  s <- summary(c[[1]])
  p_density <- create_density(c[[1]], c[[2]],c[[3]],c[[4]],c[[5]])
  p_normalise <- create_normalisation(c[[1]], c[[2]])
  print(s)
  print(p_density)
  print(p_normalise)
}
