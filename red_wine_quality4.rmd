...
create_histo <- function(chemical, chemical_name){
  p <- ggplot(aes(x=chemical), data= df) +
...
create_histo_log10 <- function(chemical, chemical_name){
  p <- create_histo(chemical)+ 
          scale_x_log10() + 
...
create_histo_sqrt <- function(chemical, chemical_name){
  p <- create_histo(chemical)+ 
          scale_x_sqrt() + 
...
```{r}
# Function: put 4 kinds of plots together
# Density, histo, histo_log10, histo_sqrt
...
create_4_plots_table <- function(chemical, chemical_name, start, end, step){
  ...
  p1 <- create_histo(chemical, chemical_name)
  p2 <- create_histo_log10(chemical, chemical_name)
  p3 <- create_histo_sqrt(chemical, chemical_name)
...
```{r}
create_normalisation <- function(chemical, chemical_name){
  p1 <- ggplot(aes(x=chemical), data= df) +
          geom_histogram(alpha = my_alpha, fill= my_color) +
          xlab(paste(chemical_name, "(original data)", sep = ' '))
...
alcohol_p_arg <- list(df$alcohol, 'Alcohol', 8, 15, 1) 
citric.acid_p_arg <- list(df$citric.acid, 'citric.acid', 0, 1, 0.1)

density <- list(df$alcohol, 'Alcohol', 8, 15, 1) 
chlorides <- list(df$alcohol, 'Alcohol', 8, 15, 1) 
fixed.acidity <- list(df$alcohol, 'Alcohol', 8, 15, 1) 
free.sulfur.dioxide <- list(df$alcohol, 'Alcohol', 8, 15, 1) 
pH <- list(df$alcohol, 'Alcohol', 8, 15, 1) 
residual.sugar <- list(df$alcohol, 'Alcohol', 8, 15, 1) 
sulphates <- list(df$alcohol, 'Alcohol', 8, 15, 1) 
volatile.acidity <- list(df$alcohol, 'Alcohol', 8, 15, 1) 
total.sulfur.dioxide <- list(df$alcohol, 'Alcohol', 8, 15, 1) 
