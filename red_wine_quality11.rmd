...
df$quality.group <-
  cut(df$quality,
      c(3,4,5,6,8),
      labels = c('Low (3,4]', 'Medium Low (4,5]', 'Medium High (5,6]', 'High (6,8]'))
...
```{r}
# Draw A Table: show info in table format
...
```
```{r}
# This chrunk of code is not used. I was tring to put the summary of chemicals into table format (create_table() function), so I can use {r results='asis'} to put the analysis into Markdown rather than output of R. But it only works outside of for loop. 
# e.g. create_table(retrive_summary(3))

# Build a data frame to store statistics of chemicals
retrive_summary <- function(i){
  # Build a new data frame
  summary_c <- data.frame(matrix(nrow = 1, ncol = 7))
  
  # Name 7 columns of table
  colnames(summary_c) <- c('Min.', '1st Qu.', 'Median', 'Mean', '3rd Qu.', 'Max.')
  
  # Store the summary into s
  # Summary (6 rows):
  # value = c(Min. = 8.4, `1st Qu.` = 9.5,...)
  s <- summary(chemicals[[i]])
  
  # Store the chemical_name / statstics into the new df
  for (j in 1:6) {
    summary_c  [1, 1] <- chemical_names[[i]]
    summary_c  [1, j+1] <- s[j]
  }
...
### Chemicals
```{r}
# {r results='asis'}----The text in the body of the report is supposed to be in Markdown (and not printed as R output).
# To achieve that, by adding the option results='asis' to the code chunk that prints (or cat()) your commentary:{r results='asis'}
...
df$fixed.acidity.group <-
  cut(df$fixed.acidity, 
      c(4.60, 7.10, 7.90, 9.20, 15.90),
      labels =c('Low (4.6,7.1]', 
                'Medium Low (7.1,7.9]', 
                'Medium High (7.9, 9.2]', 
                'High (9.2, 15.9]'))
