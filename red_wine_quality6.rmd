...
```{r}
# Plot arguments of all chemicals
...
citric.acid <- list(df$citric.acid, 
                    'Citric Acid', 
                    0, 1, 0.1)
...
fixed.acidity <- list(df$fixed.acidity, 
                      'Fixed Acidity', 
                      4, 16, 1) 

free.sulfur.dioxide <- list(df$free.sulfur.dioxide, 
                            'Free Sulfur Dioxide', 
                            1, 72, 10) 

pH <- list(df$pH, 'pH Value', 2.7, 4.1, 0.5) 

residual.sugar <- list(df$residual.sugar, 
                       'Residual Sugar', 
                       0, 5, 0.5) 
...
volatile.acidity <- list(df$volatile.acidity, 
                         'Volatile Acidity', 
                         0, 1.2, 0.2) 

total.sulfur.dioxide <- list(df$total.sulfur.dioxide, 
...
# Step 2: Draw Plot
create_scatter <- function(chemical, chemical_name, start, end, step){
...
create_violin <- function(chemical, chemical_name){
  p <-  ggplot(df_quality, 
               aes(x = quality.group, 
                   y = chemical, 
                   fill = quality.group)) + 
...
for (i in 1:11){
  p1 <- create_scatter(chemicals[[i]][[1]],
                       chemical_names[[i]],
                       chemicals[[i]][[3]],
                       chemicals[[i]][[4]],
                       chemicals[[i]][[5]])
  p2 <- create_ridgeline(chemicals_group_by_qty[[i]], chemical_names[[i]])
  p3 <- create_violin(chemicals_group_by_qty[[i]], chemical_names[[i]])
